 ...
 * the most recent project. Only returns an id that actually exists. This keeps the active project
 * consistent across navigation so you can't, say, sit in project A and accidentally act on project B.
 */
...
export async function selectProjectAction(formData: FormData): Promise<void> {
  ...
  if (projectId) (await cookies()).set(COOKIE, projectId, { httpOnly: true, sameSite: 'lax', path: '/' });
  redirect(basePath);
}
